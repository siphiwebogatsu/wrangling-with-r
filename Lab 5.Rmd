...
You’ve just been hired as a junior data analyst at a fictional streaming start-up CineMatch.
...
Create a new tibble `movies_years` from `movies` that keeps all original columns and adds a numeric `year` column extracted from `title`.
...
movie_year = movies|>
  mutate(
    year = as.numeric(str_sub(title, start = -5, end = -2))
) 
...
```{r}
# Do it here. 
# Hint: find out what is str_count() in Packages. Or just write ?str_count in your console. 
